...
int main()
{
    ...
    while(1)
    {
		...
		//CAPTURE OR CREATE THE ANALOGUE DATA TO TEST GLOBAL SUMMATION UPON
			...
			else
			{
				...
				else
				{
					...
					scamp7_in(E,127);
					scamp7_in(F,-127);
					scamp7_load_region(S6,0,0,255,input_value+127);//Load a rectangle occupying of proportion of the register plane / image
        ...
        //PERFORM GLOBAL SUMMATION USING EACH FUNCTION
			...
			sum_timer.reset();
			int16_t sum64_result = scamp7_global_sum_64(A);
			int sum64_time = sum_timer.get_usec();

			sum_timer.reset();
			int16_t sum16_result = scamp7_global_sum_16(A);
