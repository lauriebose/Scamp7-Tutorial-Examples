...
int main()
{
    ...
    while(1)
    {
	   ...
	   //CAPTURE FRAME AND PERFORM THRESHOLDING
			...
			if(flood_edges)
			{
				//Perform Flooding using native instructions
				{
					scamp7_kernel_begin();
						...
						CLR(RZ);//Copy loaded point into RZ
		...
		//STEPs 1."scamp7_scan_events" 2."scamp7_load_point" 3.Flood from point 4.process flooded shape 5.repeat
			...
			int shapes_extracted = 0;
   			for(int s = 0 ; s < shapes_to_extract ; s++)
   			{
				...
				scamp7_scan_events(S0,event_data_array,1);
				...
				{
					shapes_extracted++;
					scamp7_load_point(S6,event_ypos,event_xpos);
					...
					//Perform Flooding using native instructions
					{
						scamp7_kernel_begin();
							...
							SET(RF);//Reset digital Flag = 1 across whole processor array, as operations targeting RZ are Flagged by RF
							MOV(RZ,S6);//Copy loaded point into RZ
						...
						scamp7_kernel_begin();
							MOV(S6,RZ);//Copy result of flooding, ie the shape
					...
					//OUTPUT THE BOUNDING BOX OF THE FLOODED SHAPE
						...
						uint8_t bb_data [4];
						scamp7_scan_boundingbox(S6,bb_data);//Retrieve the bounding box of the shape in S6

						scamp7_display_boundingbox(display_00,bb_data);
