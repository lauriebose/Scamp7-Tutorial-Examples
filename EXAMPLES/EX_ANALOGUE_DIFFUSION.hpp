    ...
    while(1){
		...
		else
		{
			...
			scamp7_draw_begin(S6);
				for(int y = 0 ; y < 256 ; y+=block_size)
				{
					for(int x = 0 ; x < 256 ; x+=block_size*2)
					{
						if(x < 64 || x >= 192 || y < 64 || y >= 192)
						{
							...
							scamp7_draw_rect(y,x2,y+block_size,x2+block_size);
						}
					}
				}
				...
				{
					scamp7_draw_circle(127,127,r);
				}

				int tmp_size = 2;
				scamp7_draw_rect(127-tmp_size,127-tmp_size,127+tmp_size,127+tmp_size);
		...
		if(!use_blur_grid)
		{
			...
			scamp7_load_region(S5,y,x,y+h-1,x+w-1);
			scamp7_load_region(S6,y2,x2,y2+h2-1,x2+w2-1);
	    	scamp7_kernel_begin();
	    		OR(S3,S5,S6);
			scamp7_kernel_end();
		}
        ...
        if(vs_gui_is_on())
        {
            ...
            scamp7_output_image(RS,display_10);
		    scamp7_output_image(RW,display_11);

            scamp7_output_image(A,display_00);
            scamp7_output_image(D,display_01);
            scamp7_output_image(S4,display_02);
