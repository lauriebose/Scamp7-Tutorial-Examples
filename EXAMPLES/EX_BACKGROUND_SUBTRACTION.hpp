...
int main()
{
    ...
    //SETUP GUI ELEMENTS & CONTROLLABLE VARIABLES
		...
		{
			scamp7_in(AREG_background_update_mask,127); //Reset Background Mask
	   });
		...
		scamp7_in(AREG_background_update_mask,127);
    ...
    while(true)
    {
		...
		//UPDATE BACKGROUND MASK
			...
			if(bg_mask_update_timer.get_usec() >= bg_mask_increase_interval*1000)
			{
				...
				scamp7_in(F,bg_mask_increase_value);
				scamp7_kernel_begin();
		...
		//UPDATE BACKGROUND, BUT ONLY WHERE BACKGROUND UPDATE MASK IS HIGH VALUE
			...
			scamp7_in(F,background_update_mask_threshold);//Threshold at which to perform background update
			scamp7_kernel_begin();
		...
		//REFRESH QUANTIZED AREG STORAGE, SNAPS EACH PE'S STORED VALUE TO THE NEAREST QUANTIZED VALUE TO PREVENT VALUE DECAY/DRIFT
			...
			areg_quantization_refresh_timer.reset();
			{
				...
				{
					scamp7_in(F,decision_value);
					scamp7_in(E,current_value);
        ...
        //COMPUTE BACKGROUND SUBTRACTION MASK
			...
			scamp7_in(F,background_sub_threshold);
			scamp7_kernel_begin();
		...
		//OUTPUT IMAGES
			...
			scamp7_output_image(DREG_detected_motion,display_11);
			if(output_binary_result)
			{
				scamp7_output_image(S5,display_02);
			}
			else
			{
