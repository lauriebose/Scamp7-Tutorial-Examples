        	...
        	if(triggered_image_store != -1)
        	{
				...
				scamp7_kernel_begin();
					...
					where(D);// where D > 0
						...
						//Update bit 3 (S0)
						SET(RF);
						MOV(RM,S0); //Copy S0 to RM in all PEs
						MOV(RF,S6);
						MOV(RM,FLAG);//Copy FLAG into RM in PEs of subgrid, i.e. PEs where S6 = 1
						MOV(S0,RM);
					...
					where(E);// where D > +64
						...
						//Update bit 2 (S1)
						SET(RF);
						MOV(RM,S1); //Copy S1 to RM in all PEs
						MOV(RF,S6);
						MOV(RM,FLAG);//Copy FLAG into RM in PEs of subgrid, i.e. PEs where S6 = 1
						MOV(S1,RM);
					all();
					...
					where(E);// where D > +32
						...
						//Update bit 1 (S2)
						SET(RF);
						MOV(RM,S2); //Copy S2 to RM in all PEs
						MOV(RF,S6);
						MOV(RM,FLAG);//Copy FLAG into RM in PEs of subgrid, i.e. PEs where S6 = 1
						MOV(S2,RM);
					all();
					...
					where(E);// where D > 16
						...
						//Update bit 0 (S3)
						SET(RF);
						MOV(RM,S3); //Copy S3 to RM in all PEs
						MOV(RF,S6);
						MOV(RM,FLAG);//Copy FLAG into RM in PEs of subgrid, i.e. PEs where S6 = 1
						MOV(S3,RM);
					all();
